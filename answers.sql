...
SELECT models.name, brand_name, headquarters FROM models JOIN brands ON (models.brand_name = brands.name) WHERE models.brand_name = 'Ford' AND models.name = 'Mustang';
...
WHERE b.name NOT IN (SELECT m.brand_name FROM models AS m);
...
... [13]> FROM models AS m LEFT JOIN brands AS b
